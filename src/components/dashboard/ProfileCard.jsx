  ...
  return (
    <Card className="h-full flex flex-col items-center text-center">
      <div className="relative">
        <div className="rounded-full bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.5),transparent_45%),var(--accent-soft)] p-1.5">
          <Avatar name={user?.name} size={72} className="ring-0" />
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 w-full mt-5 pt-5 border-t border-[var(--border)]">
        {stats.map((s) => (
          <div key={s.label} className="rounded-2xl bg-[var(--surface-2)] px-2 py-2">
